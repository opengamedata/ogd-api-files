...
def get_game_file_info_by_month():
    ...
    if (_requested_game is not None) and (_requested_game in file_list_json) and (len(file_list_json[_requested_game]) > 0):
        file_info = {}
        found_matching_range = False

        _game_datasets = file_list_json[_requested_game]
